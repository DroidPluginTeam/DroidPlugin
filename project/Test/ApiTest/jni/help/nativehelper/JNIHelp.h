...
#define NATIVE_METHOD(function,functionName, signature) \
  { functionName, signature, reinterpret_cast<void*>(function) }
#endif
...
inline int jniRegisterNativeMethods2(JNIEnv* env, const char* className,
		const JNINativeMethod* gMethods, int numMethods) {
	return jniRegisterNativeMethods2(&env->functions, className, gMethods,
			numMethods);
}
...
inline int jniThrowExceptionFmt(JNIEnv* env, const char* className,
	...
	va_list args;
	va_start(args, fmt);
	int re = jniThrowExceptionFmt(&env->functions, className, fmt, args);
	va_end(args);
	return re;
}
